...
spark = SparkSession.builder \
    .appName("PyArrowToSparkDataFrame") \
    .getOrCreate()
...
pandas_table = pyarrow_table.to_pandas()
pyspark_table = spark.createDataFrame(pandas_table)


class DocQualitySparkTransform(DocQualityTransform):
...
class TestTransformSparkSignature(AbstractTableTransformTest):
    ...
    def get_test_transform_fixtures(self) -> list[Tuple]:
        ...
        return [
            (
                DocQualitySparkTransform(config),
                [pyspark_table],
                [expected_table],
                [{"total_docs_count": 1}, {}],
            )
