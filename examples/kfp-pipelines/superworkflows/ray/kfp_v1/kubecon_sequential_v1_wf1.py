...
run_tokenization1_op = comp.load_component_from_file(component_spec_path + "executeSubWorkflowComponent.yaml")
run_tokenization2_op = comp.load_component_from_file(component_spec_path + "executeSubWorkflowComponent.yaml")
...
def sample_ray_orchestrator(
    ...
    # p5_name: str = "lang_id",
    # p5_skip: bool = False,
    # p5_lang_id_model_credential: str = "PUT YOUR OWN HUGGINGFACE CREDENTIAL",
    # p5_lang_id_model_kind: str = "fasttext",
    # p5_lang_id_model_url: str = "facebook/fasttext-language-identification",
    # p5_lang_id_content_column_name: str = "text",
    ...
    + tokenization_image
    + '"}}',
    # tokenization2 step parameters
    p8_tkn_tokenizer: str = "hf-internal-testing/llama-tokenizer",
    p8_tkn_doc_id_column: str = "document_id",
    p8_tkn_doc_content_column: str = "contents",
    p8_tkn_text_lang: str = "en",
    p8_tkn_tokenizer_args: str = "cache_dir=/tmp/hf",
    p8_tkn_chunk_size: int = 0,
    # overriding parameters
    ...
    doc_quality = run_doc_quality_op(
        name=p1_orch_doc_quality_name, prefix="p5_", params=args, host=orch_host, input_folder=doc_id.output
    )
    _set_component(doc_quality, "doc quality", doc_id)

    # filtering
    filtering = run_filter_op(
        name=p1_orch_filter_name, prefix="p6_", params=args, host=orch_host, input_folder=doc_quality.output
    )
    _set_component(filtering, "filtering", doc_quality)

    # tokenization 1
    tokenization = run_tokenization1_op(
    ...
    # tokenization 2
    tokenization = run_tokenization1_op(
        name=p1_orch_tokenization_name, prefix="p8_", params=args, host=orch_host, input_folder=filtering.output
    )
    _set_component(tokenization, "tokenization", filtering)
