...
def contains_encoded_data(file_content):
    r"""
    Check if file contains inline encoded data using the following regular expressions:
        – Base64 strings: [a-zA-Z0-9+/\n=]{64,} (atleast charcter long )
        – Hexadecimal sequences: (?:\b(?:0x|\\x)?[0-9a-fA-F]{2}(?:,|\b\s*)){8,} ()atleast 8 pairs
        – Unicode strings: (?:\\u[0-9a-fA-F]{4}){8,} (\u_heaxadecimal atleats 8)
    """
    matches = []
    ...
    for match in base64_regex.finditer(file_content):
        match_length = len(match.group())
        matches.append((match, match_length))
        total_matched_length += match_length

    # Search for hexadecimal sequences
    for match in hex_regex.finditer(file_content):
    ...
    for match in unicode_regex.finditer(file_content):
        match_length = len(match.group())
        matches.append((match, match_length))
        total_matched_length += match_length


    # Check for individual match length condition
    for _, length in matches:
        if length > 1024:
            return {"contains_encoded_data":True}

    # Check for total matched characters fraction condition
    if total_matched_length > 0.5 * len(file_content):
        return {"contains_encoded_data":True}

    return {"contains_encoded_data":False}
...
class CodeQualityTransform(AbstractTableTransform):
    ...
    def transform(self, table: pa.Table, file_name: str = None) -> tuple[list[pa.Table], dict]:
        ...
        for i, c in enumerate(contents):
            ...
            contains_encoded_data_values.append(contains_encoded_data(c)["contains_encoded_data"])
        ...
        table = TransformUtils.add_column(table=table, name="contains_encoded_data", content=contains_encoded_data_values)
