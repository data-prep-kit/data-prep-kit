...
class PeopleDetect:
    ...
    def get_image_array(self, fullpath):
        ...
        imgloaded = copy.deepcopy(
            Image.open(fullpath)
        )  # check if this is needed in parallel mode to close out the readers
    ...
    def run_people_detector_objectlist(
        self, objectlist, confscore, batchsize, chatty=False
    ):
        ...
        for maxj in range(0, len(objectlist), batchsize):
            ...
            resultperbatch = self.run_people_detector_perobjectbatch(
                objectlist[maxj : (maxj + batchsize)], confscore, chatty
            )
    ...
    def run_people_detector_perobjectbatch(
        self, objectpathlist, confscore, chatty=False
    ):
        resultlist = []
        # try:
        # print(imagepathlist)
        results = self.model(
            objectpathlist, verbose=chatty, conf=confscore
    ...
    def run_people_detector_object(self, object, confscore, chatty=False):
        try:
            # start_time=datetime.now()
            results = self.model(
                object, verbose=chatty, conf=confscore
            )  # return a list of Results objects
            index = 0
    ...
    def run_people_detector_bulk(
        self, imgdir, resultfile, confscore, batchsize, chatty=False
    ):
        ...
        for root, dirs, files in os.walk(
            imgdir, topdown=False, onerror=None, followlinks=True
        ):
                ...
                if (filename != ".DS_Store") and (
                    ...
                    if total_files % batchsize == 0:
                        ...
                        if len(imagefilelist) > 0:
                            resultperbatch = self.run_people_detector_perimagebatch(
                                imagefilelist, confscore, chatty
                            )
                            cumulativeresult += resultperbatch
                            # print(len(imagefilelist),len(resultperbatch))
                            self.update_result_file(
                                resultfile, resultperbatch, imagefilelist
                            )
                            people_count += resultperbatch.count(True)
                            nonpeople_count += resultperbatch.count(
                                False
                            )  # if some failed
        ...
        print(
            "Total number of images found to have people = ",
            people_count,
            " out of ",
            total_files,
        )
        print(
            "Total number of images not found with people = ",
            nonpeople_count,
            " out of ",
            total_files,
        )
    ...
    def run_people_detector_imagelist(
        self, imagelistpath, topdir, confscore, batchsize, chatty=False
    ):
        start_time = datetime.now()
        ...
        for maxj in range(0, len(pathlist), batchsize):
            ...
            resultperbatch = self.run_people_detector_perimagebatch(
                pathlist[maxj : (maxj + 1) * batchsize], confscore, chatty
            )
            mark_images += resultperbatch
        people_count = mark_images.count(True)
        nonpeople_count = mark_images.count(False)
    ...
    def run_people_detector_perimagebatch(self, imagepathlist, confscore, chatty=False):
        ...
        try:
            # print(imagepathlist)
            results = self.model(
                imagepathlist, verbose=chatty, conf=confscore
            )  # return a list of Results objects
            index = 0
            # one result per image in the list
            for result in results:
    ...
    def run_people_detector_perimage(self, imagepath, confscore, chatty=False):
        try:
            # start_time=datetime.now()
            results = self.model(
                imagepath, verbose=chatty, conf=confscore
            )  # return a list of Results objects
            index = 0
            # single result since there is only one image
            for result in results:
    ...
    def run_people_detector_perimagedir(
        self, imagefile, imagedir, confscore, chatty=False
    ):
        # start_time=datetime.now()
        try:
    ...
    def run_people_detector_tarfiles(self, tardir, outdir, confscore):
        for root, dirs, files in os.walk(
            tardir, topdown=False, onerror=None, followlinks=True
        ):
            for filename in files:
                if ".tar" in filename:
        ...
        with tarfile.open(tarfilename, "r") as tf:
            ...
            for i in range(len(index)):
                ...
                if ".jpg" in dirorfilename:
                    ...
                    # print(type(image))
                    if not (
                        pdetect.run_people_detector_object(image, confscore, False)
                    ):
                        count_nonpeople += 1
...
if __name__ == "__main__":
    ...
    if args.mode == "image":
        if args.topdir == None:
            # boolean result
            people_present = pdetect.run_people_detector_perimage(
                args.name, args.confidence, args.verbose
            )
        # print(people_present)
        else:
            people_present = pdetect.run_people_detector_perimagedir(
                args.name, args.topdir, args.confidence, args.verbose
            )
        #  print(people_present)
    elif args.mode == "list":
        ...
        (
            ...
            nonpeople_count,
            mark_array,
        ) = pdetect.run_people_detector_imagelist(
            args.name, args.topdir, args.confidence, args.batchsize, args.verbose
        )
    # print(mark_array)
    elif args.mode == "bulk":
        # counts and list of booleans for the entire directory
        people_count, nonpeople_count, mark_array = pdetect.run_people_detector_bulk(
            args.topdir, args.name, args.confidence, args.batchsize, args.verbose
        )
        # print(mark_array)
    elif args.mode == "tar":
