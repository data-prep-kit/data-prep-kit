...
from dpk_mm.people.transform import PeopleTransform
...
class TestProtoTransform(AbstractTableTransformTest):
    ...
    def get_test_transform_fixtures(self) -> list[tuple]:
        ...
        expected_dir = os.path.join(test_data_dir, "people/expected")
        input_tables = get_tables_in_folder(input_dir)
        expected_tables = get_tables_in_folder(expected_dir)
        model_path = os.path.join(model_dir, "yolov8m-seg.pt")
        config = {"model_path": model_path }
        fixtures = [
            (PeopleTransform(config), input_tables, expected_tables, expected_metadata_list),
        ]
        return fixtures
